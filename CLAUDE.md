...
- **I5 is formally complete:** `reports/itinerary_i5_agentic_discovery.md` records a fresh Gate I5 pass. The pipeline now supports bounded agentic discovery via a single search_places tool inside the planner, enforcing 3-round/6-call limits and referential integrity against the place gateway. Total passing tests: 396.
- **I6 is formally complete:** `reports/itinerary_i6_vertical_slice.md` records a fresh Gate I6 pass. The pipeline integrates the real Overture catalog for Singapore via a mapping layer in retrieval, surfacing real venue evidence to the frontend. Live run via ScriptedLLMClient confirms catalog places. Total passing tests: 411.
- **Git is initialized:** baseline commit `b3790e8` is on `main`; remote `origin` is `https://github.com/hjain3004/TripPlanner.git`; current work is on `feat/catalog-retrieval-i6`, which now carries both the I6 frontend commits and the 2026-08-14 spec/plan docs. Never claim a push or PR until it actually succeeds. Untracked by choice: `frontend/design/probes/` (6.4 MB of design-probe binaries; Wikimedia sources are CC BY-SA and need attribution before committing) and `frontend/design/pipeline/`.
