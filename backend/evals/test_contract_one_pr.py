...
def git_changed_files_in_head() -> list[str]:
    ...
    # "contract changed without [...] "IN THE SAME COMMIT" -> it means the currently uncommitted
    # or staged files during `make gate`? 
    # `git status --porcelain` might be what we want,
    # OR `git diff --cached --name-only` + `git diff --name-only`.
    # So let's implement "git_changed_files_in_head()".
