    ...
    draft = DraftItinerary(
        ...
        days=[
            ItineraryDay(
                date=date(2026, 8, 3),
                items=[
                    ItineraryItem(poi_id="invented_place", start_time="09:00", end_time="10:00")
                ],
            )
        ],
    )
...
def test_closed_day_is_rejected() -> None:
    draft = DraftItinerary(
        ...
        days=[
            ItineraryDay(
                date=date(2026, 8, 3),
                items=[ItineraryItem(poi_id="p1", start_time="09:00", end_time="10:00")],
            )
        ],
    )
    ...
    draft = DraftItinerary(
        ...
        days=[
            ItineraryDay(
                ...
                items=[
                    ItineraryItem(
                        poi_id="p1", start_time="09:00", end_time="10:00", start_hint="09:00"
                    )
                ],
            )
        ],
...
def test_fixed_window_violated() -> None:
    draft = DraftItinerary(
        ...
        days=[
            ItineraryDay(
                date=date(2026, 8, 3),
                items=[ItineraryItem(poi_id="p1", start_time="11:00", end_time="12:00")],
            )
        ],
    )
    poi = _poi("p1")
...
def test_fresher_evidence_is_preferred_between_equal_candidates() -> None:
    ...
    assert (
        scheduled.index("pl_fresh") <= scheduled.index("pl_stale")
        if "pl_stale" in scheduled
        else True
    )
