...
def test_dangling_superseded_pointer_is_a_violation(claim_a: Claim, source_a: Source) -> None:
...
def test_superseded_by_missing_edge_is_a_violation(claim_a: Claim, source_a: Source) -> None:
    g = EvidenceGraph()
    g.add_source(source_a)
    claim_a2 = claim_a.model_copy(update={"claim_id": "c-a2"})
    g.add_claim(
        claim_a.model_copy(update={"lifecycle": LifecycleState.SUPERSEDED, "superseded_by": "c-a2"})
    )
...
def test_supersedes_edge_missing_field_is_a_violation(claim_a: Claim, source_a: Source) -> None:
    g = EvidenceGraph()
    g.add_source(source_a)
    claim_a2 = claim_a.model_copy(update={"claim_id": "c-a2"})
...
def test_artifact_can_derive_from_another_artifact(claim_a: Claim, source_a: Source) -> None:
    g = EvidenceGraph()
    g.add_source(source_a)
    g.add_claim(claim_a)
    a1 = Artifact(
        artifact_id="a1", kind="OptimizerResult", run_id="r1", version=1, derived_from=["c-a"]
    )
    a2 = Artifact(artifact_id="a2", kind="CostedTrip", run_id="r1", version=1, derived_from=["a1"])
    g.add_artifact(a1)
    g.add_artifact(a2)
    violations = check_invariants(g)
    assert violations == []
...
def test_artifact_can_derive_from_kb_facts(claim_a: Claim, source_a: Source) -> None:
    g = EvidenceGraph()
    g.add_source(source_a)
    g.add_claim(claim_a)
    a1 = Artifact(
        artifact_id="a1",
        kind="OptimizerResult",
        run_id="r1",
        version=1,
        derived_from=["c-a"],
...
def test_resolution_missing_created_by_run_is_violation(claim_a: Claim, source_a: Source) -> None:
    ...
    g.add_source(source_a)
    g.add_claim(claim_a)
    g.resolutions["res:c-a"] = ResolutionRecord(
        resolution_id="res:c-a",
        members=["c-a", "c-b"],
        canonical_id="c-a",
        rule="exact_identity",
        confidence=1.0,
        created_by_run="missing-run",
        state=ResolutionState.ACTIVE,
    )
    violations = check_invariants(g)
    assert any("created_by_run missing" in v for v in violations)


def test_resolution_missing_member_is_violation(claim_a: Claim, source_a: Source) -> None:
    g = EvidenceGraph()
    g.runs["r1"] = Run(run_id="r1", started_at="2026-10-12T10:00:00Z")
    g.add_source(source_a)
    g.add_claim(claim_a)
    g.resolutions["res:c-a"] = ResolutionRecord(
        resolution_id="res:c-a",
        members=["c-a", "missing-claim"],
        canonical_id="c-a",
        rule="exact_identity",
        confidence=1.0,
        created_by_run="r1",
    ...
    violations = check_invariants(g)
    assert any("member missing" in v for v in violations)
