...
def test_artifact_requires_run_and_version() -> None:
    with pytest.raises(ValidationError):
        Artifact(artifact_id="a1", kind="CostedTrip", run_id="", version=1, derived_from=["c1"])
...
def test_superseding_a_verify_required_claim_preserves_the_safety_signal(claim_a, source_a):
    """verify_required must survive supersession — it is a Tier-F safety signal."""
    g = EvidenceGraph()
    g.add_source(source_a)
    g.add_claim(claim_a.model_copy(update={"status": FreshnessState.VERIFY_REQUIRED}))
