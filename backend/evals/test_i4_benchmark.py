...
def _matrix() -> RouteMatrix:
    return RouteMatrix(cells=[
        RouteCell(origin_place_id="a", destination_place_id="b", mode="transit", duration_min=15, distance_km=1, retrieved_at=datetime.now(UTC), source="geodesic_estimate", status="estimated", confidence=0.9),
    ])
...
def test_ortools_outperforms_or_matches_greedy() -> None:
    ...
    assert ortools_score >= greedy_score, f"ORTools ({ortools_score}) worse than Greedy ({greedy_score})"
