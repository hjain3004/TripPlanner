...
def validate_draft(
    ...
    for day in draft.days:
        ...
        for item in day.items:
            ...
            if not poi:
                rejections.append(
                    RejectionReason(code="no_evidence_backed_place_id", place_id=item.poi_id)
                )
                prev_item = item
                continue
            ...
            if status == "closed":
                rejections.append(
                    RejectionReason(
                        code="closed_day", place_id=item.poi_id, detail="explicitly closed"
                    )
                )
            elif status == "unknown" and item.start_hint:
                rejections.append(
                    RejectionReason(code="unknown_hours_timing_critical", place_id=item.poi_id)
                )

            if "inaccessible" in poi.tags:
                rejections.append(
                    RejectionReason(code="accessibility_excluded", place_id=item.poi_id)
                )
                ...
                if tag.startswith("fixed_window_"):
                    ...
                    if len(parts) == 4:
                        ...
                        if item.start_time != sw or item.end_time != ew:
                            rejections.append(
                                RejectionReason(code="fixed_window_violated", place_id=item.poi_id)
                            )
        ...
        if daily_travel_min > constraints.max_daily_travel_min:
            rejections.append(
                RejectionReason(
                    code="travel_budget_exceeded",
                    detail=f"{daily_travel_min} > {constraints.max_daily_travel_min}",
                )
            )
