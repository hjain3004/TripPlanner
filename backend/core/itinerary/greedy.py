...
class GreedyComposer:
    ...
    def compose(
        ...
        def poi_score(poi):
            ...
            if any(t in poi.tags for t in ["must_do_tag", "must_do", "must_see"]): sc += 500
            if poi.provenance and poi.provenance.last_verified:
                sc += poi.provenance.last_verified.toordinal() // 100
            return sc
            
        # Tiebreaker: (poi.area != hotel_area_id, poi.area, poi.id) ensures exact match with I1 composer
        pois = sorted(retrieval.pois, key=lambda poi: (-poi_score(poi), poi.area != hotel_area_id, poi.area, poi.id))
