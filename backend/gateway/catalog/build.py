        ...
        if fail_quality:
            ...
            for cat, min_count in _MIN_PER_CATEGORY.items():
                for _ in range(min_count):
                    ...
                    raw_claims.append(
                        PlaceClaim(place_id=p_id, field="category", value=cat, **base_args)
                    )
                    raw_claims.append(
                        PlaceClaim(
                            place_id=p_id,
                    ...
                    raw_claims.append(
                        PlaceClaim(place_id=p_id, field="opening_hours", value="24/7", **base_args)
                    )
                    place_idx += 1
            ...
            raw_claims.append(
                PlaceClaim(place_id=p_id, field="category", value="park", **base_args)
            )
            raw_claims.append(
                PlaceClaim(
                    place_id=p_id, field="coordinates", value={"lat": 1, "lon": 1}, **base_args
                )
            )
