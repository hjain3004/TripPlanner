...
- **Retrieval (code, not LLM):** `kb.pois(city, tags=interests)` ranked by tag overlap and provenance trust (`needs_verification`, `verified_by`, `last_verified`, `confidence`); take top ~40; plus the `areas` table. Compact each POI to one line: `id | name | area | tags | price | duration | hours | trust_status`. This is the context-engineering step: the LLM sees only curated candidates and their trust levels, so it cannot invent attractions and is aware of data provenance.
