...
Accounts & acquisition: spec **17** (accounts + persistence) is authoritative design; `docs/superpowers/plans/2026-07-28-accounts-persistence.md` was written before it — a deliberate, human-approved process inversion logged as SCOPE+ in `DEVIATIONS.md`; where the two disagree, spec 17 wins. Backend **A1** persistence and **A2** email/password authentication are implemented, but account UI and wiring stored wallets/trips into the planning flow remain unimplemented. Spec **18** (card acquisition + welcome offers) is not implemented and depends on the remaining spec-17 product wiring. Spec 18's Case A (welcome window on an already-held card) is shippable independently of Case B (new-card suggestion) — do not build Case B first.
...
- **Backend regression baseline is 717 tests.** Strict mypy clean across 100 source files.
- **Backend accounts A1/A2 are integrated:** isolated SQLite user persistence, wallet projection, immutable saved-trip revisions, privacy export/delete, Argon2id credentials, server-side hashed-token sessions, lockout, cookie authentication and CSRF-protected logout are implemented. Auth routes are synchronized through OpenAPI, generated frontend types and MSW fixtures. Password changes revoke existing sessions; CORS explicitly permits the CSRF header. Account UI and planning-flow persistence remain future work.
- **Audit remediation is formally complete:** `reports/audit_remediation_execution.md` records remediation of the consensus audit items. Runtime LLM/provider failures are now truthful `PipelineStatus.ERROR` responses instead of user clarification prompts, while schema-repair failures and missing replay fixtures remain clarification/replay workflow states. A canonical travel taxonomy (`backend/core/travel_taxonomy.py`) now aligns user interests, picker filters, provider categories and POI spend categories without changing raw provider category/provenance fields. Real activated catalogs across SIN/BOM/DXB/NYC/LON/PAR now produce interest-sensitive retrieval; picker-visible filters return non-empty matching catalog venues; restaurants/cafes/food courts cost as dining; warm real-catalog recompute is cache-backed; empty-card wallets return explicit `cash_only` zero-rewards output; regional POIs use deterministic `geo-cell:*` area labels rather than `Unknown`; exact venue search prefers category-compatible and geographically plausible candidates. No paid provider, runtime crawling, golden money math, OpenAPI contract, backend/core gateway import, or provider/MCP activation change was made.
- **Post-remediation live LLM smoke passed:** `reports/audit_remediation_execution.md` records a bounded live product smoke through the real FastAPI `/plan` path using existing ignored `backend/.env` free-tier LLM credentials. The configured `.env` primary model `llama-3.3-70b-versatile` is no longer listed by the provider, so the smoke used command-local `TRIPWISE_LLM_MODEL=openai/gpt-oss-20b` with `TRIPWISE_LLM_FALLBACK_MODELS=qwen/qwen3.6-27b` and `TRIPWISE_LLM_MAX_CALLS=6`; it completed in `70.1s` at stage `explaining` with `has_report=true`. Trace metadata showed planner repair plus deterministic planner fallback (`used_fallback=True`), which is expected product behavior for weak tool-calling output. No secrets, raw provider responses, paid travel providers, Tripadvisor live transport, bookings, crawling, or MCP runtime activation were used.
...
- **Real LLM Integration:** `HostedFreeTier` provides OpenAI-compatible HTTP integration using `TRIPWISE_LLM_MODEL` plus optional `TRIPWISE_LLM_FALLBACK_MODELS` or an ordered `TRIPWISE_LLM_MODELS` list. Provider bodies and bearer tokens are redacted from errors; 404 model failures can fall through to the next configured free-tier model within the per-instance call ceiling. Zero network calls in the normal test suite via replay fixtures.
...
- **Git status:** Branch is `feat/i8a-tripadvisor-offline-adapter`; latest in-scope work is post-remediation live LLM smoke documentation.
